...
export class SchemaValidator {
   ...
   */
  validateAgainstDomainSchemas(
    data: any,
  ): ValidationResult & { detectedDomain?: string } {
    const domainSchemas = [
      "order",
      "invoice",
      "booking",
  ...
  validateWithSuggestions(
    ...
    if (result.errors) {
      for (const error of result.errors) {
        if (error.keyword === "required") {
          suggestions.push(
            `Add required field: ${error.params?.missingProperty}`,
          );
        } else if (error.keyword === "type") {
          suggestions.push(
          ...
          suggestions.push(
            `Field '${error.instancePath}' should match format '${error.params?.format}'`,
          );
        }
      }
    }
