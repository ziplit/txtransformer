...
export class SchemaLoader {
  ...
  getSchemasByDomain(domain: string): SchemaInfo[] {
    return Array.from(this.loadedSchemas.values())
      .filter(schema => schema.domain === domain);
  }
  ...
  getDomainSchemas(): SchemaInfo[] {
    const domainTypes = ['order', 'invoice', 'booking', 'signature_request', 'alert', 'support_followup', 'generic'];
    return Array.from(this.loadedSchemas.values())
      .filter(schema => domainTypes.includes(schema.domain));
  }
}
